...
class CppClassAnalyzer(TypeAnalyzer):
	...
	def search_func(self, func_addr):
		...
		for w in func_fav.iterate_var_assigns(var):
			...
			vtbl = self.vtbl_analyzer.analyze_var(Var(intval))
			if vtbl is None:
				continue
	...
	def search_vtable(self, vtbl):
		if isinstance(vtbl, int):
			addr = vtbl
			vtbl = self.vtbl_analyzer.analyze_var(Var(addr))
	...
	def check_path(self, cdtor, ctors, dtors):
		...
		if utils.got_path(cdtor.get_ea(), ctors):
			cdtor._is_ctor = True
		elif utils.got_path(cdtor.get_ea(), dtors):
			cdtor._is_dtor = True
